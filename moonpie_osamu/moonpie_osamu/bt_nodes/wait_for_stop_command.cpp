...
class WaitForStopCommand : public BT::SyncActionNode {
public:
  WaitForStopCommand(const std::string& name, const BT::NodeConfiguration& config)
    : BT::SyncActionNode(name, config), node_(rclcpp::Node::make_shared("wait_for_stop_command")), stopped_(false) {
    sub_ = node_->create_subscription<std_msgs::msg::Empty>(
      "/mission/stop", 10, [this](const std_msgs::msg::Empty::SharedPtr) { stopped_ = true; });
    sub_cmd_ = node_->create_subscription<moonpie_osamu::msg::MissionCommand>(
      "/mission/cmd", 10, [this](const moonpie_osamu::msg::MissionCommand::SharedPtr msg) {
        if (msg->command == "STOP") stopped_ = true;
      });
    ...
    msg.current_node = "WaitForStopCommand";
    msg.status = "WAITING";
    msg.details = "Waiting for stop command";
    msg.message = "";
    pub_->publish(msg);
  }
  ~WaitForStopCommand() { node_->shutdown(); if (spin_thread_.joinable()) spin_thread_.join(); }
  BT::NodeStatus tick() override {
    if (stopped_) {
      ...
      msg.current_node = "WaitForStopCommand";
      msg.status = "STOPPED";
      msg.details = "Received stop command";
      msg.message = "";
      pub_->publish(msg);
