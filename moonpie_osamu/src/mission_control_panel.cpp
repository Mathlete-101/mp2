  ...
  connection_status_(ConnectionStatus::DISCONNECTED)
{
  ...
  connect(startDiggingBtn, &QPushButton::clicked, this, &MissionControlPanel::onStartDigClicked);
  connect(stopDiggingBtn, &QPushButton::clicked, this, &MissionControlPanel::onStopDigClicked);
  connect(digAndDumpBtn, &QPushButton::clicked, this, &MissionControlPanel::onStartDigAndDumpClicked);
  connect(digAndDumpX3Btn, &QPushButton::clicked, this, &MissionControlPanel::onStartDigAndDumpX3Clicked);
  connect(sendConfigBtn, &QPushButton::clicked, this, &MissionControlPanel::onConfigClicked);
...
void MissionControlPanel::onBehaviorStatus(const moonpie_osamu::msg::BehaviorStatus::SharedPtr msg)
{
  ...
  // Update status if it's from the arduino control node
  if (msg->current_node == "arduino_control") {
    QString statusText = QString::fromStdString(msg->status);
    diggingStatusLabel->setText("Status: " + statusText);
    
    // Update status label color based on the state
    if (statusText == "TELEOP") {
...
void MissionControlPanel::onStartDigClicked()
{
    auto msg = std::make_unique<moonpie_osamu::msg::MissionCommand>();
...
void MissionControlPanel::onStartDigAndDumpClicked()
{
    auto msg = std::make_unique<moonpie_osamu::msg::MissionCommand>();
...
void MissionControlPanel::onStartDigAndDumpX3Clicked()
{
    auto msg = std::make_unique<moonpie_osamu::msg::MissionCommand>();
...
void MissionControlPanel::onStopDigClicked()
{
    auto msg = std::make_unique<moonpie_osamu::msg::MissionCommand>();
...
void MissionControlPanel::onConfigClicked()
{
    auto msg = std::make_unique<moonpie_osamu::msg::MissionCommand>();
    msg->command = "CONFIG";
    msg->dig_time = digTimeSpinBox->value();
